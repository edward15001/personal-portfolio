...
(() => {
    ...
    if (gsap && ScrollTrigger) {
        ...
        /* ——— Hero intro timeline ——— */
        const heroLetters = gsap.utils.toArray('.hero__letter') as HTMLElement[];
        ...
        /* ——— Animated counters ——— */
        (gsap.utils.toArray('.counter') as HTMLElement[]).forEach((el) => {
        ...
        (gsap.utils.toArray('.showcase-item') as HTMLElement[]).forEach((item, i) => {
            const even = i % 2 === 1;
            const content = item.querySelector<HTMLElement>('.showcase-item__content');
            const media = item.querySelector<HTMLElement>('.showcase-item__media');
            const num = item.querySelector<HTMLElement>('.showcase-item__num');
            const img = media?.querySelector<HTMLElement>('img') ?? null;
            const parts = [
                item.querySelector<HTMLElement>('.showcase-item__title'),
                item.querySelector<HTMLElement>('.showcase-item__desc'),
                item.querySelector<HTMLElement>('.showcase-item__meta'),
                item.querySelector<HTMLElement>('.showcase-item__link'),
            ].filter((el): el is HTMLElement => el !== null);

            if (content) {
                gsap.fromTo(
                    content,
                    { xPercent: even ? 8 : -8, opacity: 0 },
                    {
                        xPercent: 0,
                        opacity: 1,
                        ease: 'none',
            ...
            if (img) {
                gsap.fromTo(
                    img,
                    { scale: even ? 1.22 : 1.12, xPercent: even ? 4 : -4 },
                    {
                        scale: 1,
                        xPercent: 0,
                        ease: 'none',
            ...
            if (num) {
                gsap.fromTo(
                    num,
                    { opacity: 0.08, xPercent: even ? 14 : -14 },
                    {
                        opacity: 0.9,
                        xPercent: 0,
                        ease: 'none',
