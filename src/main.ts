...
(() => {
    ...
    const initHeroParticles = (): void => {
        const canvas = document.querySelector<HTMLCanvasElement>('.hero__particles');
        const text = document.querySelector<HTMLElement>('.hero__title-text');
        const hero = document.querySelector<HTMLElement>('.hero');
        if (!canvas || !text || !hero) return;
        ...
        const build = (): void => {
            ...
            for (let y = 0; y < off.height; y += gap) {
                for (let x = 0; x < off.width; x += gap) {
                    if (data[(y * off.width + x) * 4 + 3] > 120) targets.push({ x, y });
                }
            }
